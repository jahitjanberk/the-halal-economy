...
export function drawSectorBars(){
  ...
  rows.append('rect').attr('x',mL).attr('y',6).attr('height',18).attr('rx',4).attr('fill',d=>d.key==='travel'?'#B8912F':'#1F7A63').attr('width',0).transition().duration(reduceMotion?0:700).ease(d3.easeCubicOut).attr('width',d=>x(d['v'+year])-mL);
  ...
  svg.append('text').attr('x',narrow?0:mL).attr('y',H-8).attr('fill','#6E837D').attr('font-size',11).text(narrow?'Gold: fastest-growing sector. Scale fixed.':'Gold: fastest-growing sector. Scale fixed so the two years compare directly.');
