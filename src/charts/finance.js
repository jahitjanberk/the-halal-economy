...
export function drawCompositionDonut(){
  ...
  const cols=['#1F7A63',GOLD_FILL,'#8FC3B2']; const arc=d3.arc().innerRadius(R-30).outerRadius(R).padAngle(.02).cornerRadius(3); const pie=d3.pie().value(d=>d.v).sort(null)(finComposition);
  ...
  g.append('text').attr('text-anchor','middle').attr('y',18).attr('font-size',11.5).attr('fill',MUTED).text('total assets, 2024');
...
export function drawSegmentGrowth(){
  ...
  rows.append('rect').attr('x',mL).attr('y',22).attr('height',16).attr('rx',4).attr('fill',(d,i)=>i===0?GOLD_FILL:'#1F7A63').attr('width',0).transition().duration(reduceMotion?0:800).attr('width',d=>x(d.v)-mL);
  rows.append('text').attr('class','bar-val').attr('x',d=>x(d.v)+8).attr('y',35).text(d=>'+'+d.v+'%');
  svg.append('text').attr('x',0).attr('y',H-14).attr('font-size',10.5).attr('fill',MUTED).text('IFSB Stability Report 2025. Sukuk = issuance volume.');
...
export function drawFinanceShare(){
  ...
  svg.append('text').attr('x',0).attr('y',H-14).attr('font-size',10.5).attr('fill',MUTED).text('Ten countries hold about 95% of global assets.');
